...
# Lade .env aus mehreren Orten (rank_bot Ordner und parent Deadlock Ordner)
load_dotenv()  # Aktuelles Verzeichnis
load_dotenv(os.path.join(os.path.dirname(__file__), '.env'))  # rank_bot/.env
load_dotenv(os.path.join(os.path.dirname(__file__), '..', '.env'))  # Deadlock/.env
...
if __name__ == "__main__":
    init_database()
    # Verwende das normale Discord Token (gleiches wie Main Bot)
    token = os.getenv('DISCORD_RANK_BOT_TOKEN') or os.getenv('DISCORD_TOKEN')
    if not token:
        print("❌ FEHLER: Kein Discord Token gefunden!")
        print("Setze DISCORD_TOKEN oder DISCORD_RANK_BOT_TOKEN in der .env Datei")
        exit(1)
    bot.run(token)