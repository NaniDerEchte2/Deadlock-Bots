...
class TempVoiceUtil:
    ...
    async def ban(self, lane: discord.VoiceChannel, owner_id: int, raw: str) -> tuple[bool, str]:
        ...
        if not uid:
            return (
                False,
                f"Konnte den Nutzer nicht finden. {err_msg or 'Bitte @Erwähnung oder ID angeben.'}",
            )
        ...
        try:
            ...
            logger.warning(
                "Ban-Persistenz fehlgeschlagen (owner=%s, target=%s): %r",
                owner_id,
                uid,
                e,
            )
        ...
        if target_member:
            try:
                await lane.set_permissions(
                    target_member, connect=False, reason="TempVoice: Owner-Ban"
                )
                overwrite_applied = True
            except discord.Forbidden:
                logger.warning(
                    "Owner-Ban set_permissions: fehlende Rechte (owner=%s, target=%s, lane=%s)",
                    owner_id,
                    uid,
                    lane.id,
                )
                ...
                logger.error(
                    "Owner-Ban set_permissions HTTPException (owner=%s, target=%s, lane=%s): %s",
                    owner_id,
                    uid,
                    lane.id,
                    e,
                ...
                logger.exception(
                    "Owner-Ban set_permissions: unerwarteter Fehler (owner=%s, target=%s, lane=%s)",
                    owner_id,
                    uid,
                    lane.id,
                )
            ...
            logger.debug(
                "Owner-Ban: Member %s nicht in Guild %s - nur Persistenz",
                uid,
                lane.guild.id,
            )
    ...
    async def unban(self, lane: discord.VoiceChannel, owner_id: int, raw: str) -> tuple[bool, str]:
        ...
        try:
            ...
            logger.warning(
                "Unban-Persistenz fehlgeschlagen (owner=%s, target=%s): %r",
                owner_id,
                uid,
                e,
            )
        ...
        if target_member:
            try:
                await lane.set_permissions(
                    target_member, overwrite=None, reason="TempVoice: Owner-Unban"
                )
                return True, "User entbannt."
            except discord.Forbidden:
                logger.warning(
                    "Owner-Unban set_permissions: fehlende Rechte (owner=%s, target=%s, lane=%s)",
                    owner_id,
                    uid,
                    lane.id,
                )
                ...
                logger.error(
                    "Owner-Unban set_permissions HTTPException (owner=%s, target=%s, lane=%s): %s",
                    owner_id,
                    uid,
                    lane.id,
                    e,
                ...
                logger.exception(
                    "Owner-Unban set_permissions: unerwarteter Fehler (owner=%s, target=%s, lane=%s)",
                    owner_id,
                    uid,
                    lane.id,
                )
        ...
        logger.debug(
            "Owner-Unban: Member %s nicht in Guild %s - nur Datenbankeintrag entfernt",
            uid,
            lane.guild.id,
        )
    ...
    async def toggle_lurker(
        ...
        if not role:
            return (
                False,
                "Die konfigurierte Lurker-Rolle (ID 1447747896253485127) existiert nicht auf dem Server.",
            )
        ...
        if existing_lurker_data:  # User is currently a lurker, so remove
            ...
            if lane.user_limit > 0:
                ...
                await self.core.safe_edit_channel(
                    lane, desired_limit=new_limit, reason="TempVoice: Lurker removed"
                )
        ...
        if lane.user_limit > 0:
            ...
            await self.core.safe_edit_channel(
                lane, desired_limit=new_limit, reason="TempVoice: Lurker added"
            )
