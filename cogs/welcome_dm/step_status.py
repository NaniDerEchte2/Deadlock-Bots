...
class PlayerStatusView(StepView):
    ...
    async def status_select(
        ...
        try:
            ...
            logger.warning(
                "Edit message fehlgeschlagen (user=%s): %s",
                getattr(interaction.user, "id", "?"),
                e,
            )
        except Exception:
            logger.exception(
                "Unerwarteter Fehler beim Edit der Status-Nachricht (user=%s)",
                getattr(interaction.user, "id", "?"),
            )
            ...
            try:
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        "Bitte wähle zuerst eine Option.", ephemeral=True
                    )
                else:
                ...
                logger.warning(
                    "Hinweis senden fehlgeschlagen (user=%s): %s",
                    getattr(interaction.user, "id", "?"),
                    e,
                )
            except Exception:
                logger.exception(
                    "Unerwarteter Fehler beim Senden des Hinweises (user=%s)",
                    getattr(interaction.user, "id", "?"),
                )
            return
