...
USERS = {
    ...
    "naniderechte": {"id": 662995601738170389, "heroes": []},
    "hucci1789": {
...
class ClaimView(discord.ui.View):
    ...
    async def claim_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if (
            str(self.thread_id) in self.cog.claimed_threads
            and self.cog.claimed_threads[str(self.thread_id)] is not None
        ):
            await interaction.response.send_message(
        ...
        if interaction.user.id != self.assigned_user_id and self.assigned_user_id != 0:
            await interaction.response.send_message(
                "Dieses Coaching ist einem anderen Benutzer zugewiesen.", ephemeral=True
            )
            return
    ...
    async def release_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.channel.send(
            f"<@&{STAFF_ROLE_ID}> Dieses Coaching wurde freigegeben und kann von jedem übernommen werden."
        )
        await interaction.response.send_message(
            "Du hast dieses Coaching erfolgreich freigegeben.", ephemeral=True
        )
...
class ClaimSystem(commands.Cog):
            ...
            try:
                ...
                if e.errno == 10048 or e.errno == errno.EADDRINUSE:
                    if attempt < max_retries - 1:
                        log.debug(
                            f"ClaimSystem port {SOCKET_PORT} belegt, retry in {retry_delay}s..."
                        )
    ...
    async def process_notification_data(self, data: dict[str, Any]):
        try:
            ...
            try:
                assigned_user = self.bot.get_user(
                    int(assigned_user_id)
                ) or await self.bot.fetch_user(int(assigned_user_id))
                if assigned_user:
