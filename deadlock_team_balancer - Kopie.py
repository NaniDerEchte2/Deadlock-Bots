...
class DeadlockTeamBalancer(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # Kategorie für Match-Channels (gleich wie rank_voice_manager)
        self.match_category_id = 1357422957017698478
    ...
    async def end_match(self, ctx, match_id: str = None, skip_debrief: bool = False):
        """Beendet ein Match und sammelt alle Spieler für Nachbesprechung"""
        if not match_id:
        ...
        # 1. Erstelle TempVoice Channel für Nachbesprechung (falls nicht übersprungen)
        tempvoice_channel = None
        moved_to_tempvoice = 0
        failed_tempvoice_moves = []
        
        if not skip_debrief:
            tempvoice_channel = await self.trigger_tempvoice_creation(ctx, match_id)
            
            if tempvoice_channel:
                # 2. Bewege alle Spieler zur TempVoice Nachbesprechung
                moved_to_tempvoice, failed_tempvoice_moves = await self.move_players_to_tempvoice(
                    match_data, tempvoice_channel, ctx
                )
        ...
        embed = discord.Embed(
            ...
            color=discord.Color.green() if tempvoice_channel else discord.Color.red()
        )
        
        if tempvoice_channel:
            embed.add_field(
                name="💬 TempVoice Nachbesprechung",
                value=f"**Channel**: {tempvoice_channel.mention}\n"
                      f"**Spieler bewegt**: {moved_to_tempvoice}/{len(match_data['players'])}\n"
                      f"*TempVoice Channel wurde über Casual Lane erstellt und wird automatisch verwaltet*",
                inline=False
            )
            
            if failed_tempvoice_moves:
                embed.add_field(
                    name="⚠️ Nicht zur Nachbesprechung bewegt",
                    value="\n".join(failed_tempvoice_moves[:5]) + (f"\n... und {len(failed_tempvoice_moves)-5} weitere" if len(failed_tempvoice_moves) > 5 else ""),
                    inline=False
                )
        else:
            embed.add_field(
                name="❌ TempVoice Nachbesprechung",
                value="TempVoice Channel konnte nicht erstellt werden (Casual Lane Problem?)",
                inline=False
            )
        ...
        await ctx.send(embed=embed)
        logger.info(f"Match {match_id} beendet - {len(deleted_channels)} Channels gelöscht, {moved_to_tempvoice} Spieler zur TempVoice bewegt")
